...
class Physics:
    ...
    def n_matrix(a: float, k: float) -> np.ndarray:
        out_matrix = np.array([[exp(1j * k * a), 0.0],
                               [0.0, exp(-1j * k * a)]], np.dtype(np.complex128))
        return out_matrix

    @staticmethod
    def n_z_matrix(z: float, z0: float, k: float) -> np.ndarray:
        out_matrix = np.array([[exp(1j * k * (z - z0)), 0.0],
                               [0.0, exp(-1j * k * (z - z0))]], np.dtype(np.complex128))
        return out_matrix
    ...
    def find_wave_function(energy: float, z: float) -> List:
        ...
        n1_1 = Physics.n_matrix(CONST.WIDTH_FIRST_QW, k1)
        n2_1 = Physics.n_matrix(CONST.WIDTH_BARRIERS, k2)
        n3_1 = Physics.n_matrix(CONST.WIDTH_SECOND_QW, k3)
